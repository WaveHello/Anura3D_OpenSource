!*************************************************************************************
    ...
    integer(INTEGER_TYPE) :: IError, ISurf, I, J, num_contact_surfaces, NodID, IEl, NFoundNodes, CheckNodeID,           &
                             ...
                             local_side_id, IRef, node_1, node_2
    ...
    integer(INTEGER_TYPE), dimension(:)  , allocatable :: contact_surf_element_id, Sides, NNodes, MatNodes, node_1_coord, node_2_coord

    real(REAL_TYPE)      , dimension(:,:), allocatable :: TotSrfaceAndLoad, ContactVolumes, ContactVolumesDummy, NNormals,&
                                                          NNormals2,MatNodesCood, DVectors
    ...
    integer(INTEGER_TYPE), dimension(:)  , allocatable :: local_corner_node_ids
        ...
        allocate(NNodes(ELEMENTNODES), stat = IError) ! ??
        allocate(DVectors(ELEMENTNODES,2), stat = IError) ! Dummy vector used to rotate tangent vector to get normal vector
        allocate(NNormals(ELEMENTNODES,2), stat = IError) ! Node Normals?
        ...
        do I=1,num_contact_surfaces
            ...
            ! Store the global node ids
            NNodes(1:ELEMENTNODES) = ElementConnectivities(1:ELEMENTNODES, ElementID)
            ...
            ! Getting the tangent vector to the element side
            ! If any of the nodes

            do J=1,num_element_sides
                ! Get the index where NNodes == 0, this is the element node that is 
                node_1 = ContactSurfaceElmNodes(I,N
                node_2 = 
                ...
                else if (ELEMENTTYPE == QUAD4) then
                    if (NNodes(J)==0) then
                        if (J==1) then
                            node_1_coord = NodalCoordinates(ContactSurfaceElmNodes(I,NNodes(3)),1:2)
                            node_2_coord = NodalCoordinates(ContactSurfaceElmNodes(I,NNodes(2)),1:2)
            ...
            if (IRef/=0) then
                global_material_id=ElementMaterialID(IRef) !Material Index
