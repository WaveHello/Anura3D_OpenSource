    ...
    real(REAL_TYPE), allocatable :: SigmaEffArrayGaussPoints(:,:,:) !> Gauss point integration using quadrilateral elements --> solid skeleton --> Rank 3 because tensor
    real(REAL_TYPE), allocatable :: SigmaEffArrayGaussPointsWaterPressure(:,:) !> Gauss point integration using quadrilateral elements --> water pressure --> Rank 2 because scalar
    real(REAL_TYPE), allocatable :: SigmaEffArrayGaussPointsGasPressure(:,:) !> Gauss point integration using quadrilateral elements --> gas pressure --> Rank 2 because scalar
    real(REAL_TYPE), allocatable :: SigmaEffArrayGaussPointsBulkViscousPressure(:,:) !> Gauss point integration using quadrilateral elements --> bulk viscous damping
    ...
    ! stress for gauss point integration for each phase
    allocate(SigmaEffArrayGaussPoints             (Counters%NEl, Counters%NGaussPoints, NTENSOR), stat = IError) !--> solid skeleton --> tensor for each GP
    allocate(SigmaEffArrayGaussPointsWaterPressure(Counters%NEl, Counters%NGaussPoints), stat = IError) !--> water pressure --> scalar for each GP
    allocate(SigmaEffArrayGaussPointsGasPressure  (Counters%NEl, Counters%NGaussPoints), stat = IError) !--> gas pressure --> scalar for each GP
    allocate(SigmaEffArrayGaussPointsBulkViscousPressure(Counters%NEl, Counters%NGaussPoints), stat = IError) !--> bulk viscous damping

    ! in each element, we are tracking how many material points associated with a gauss point in an element
    allocate(SubElementMPOrganization(Counters%NEl, Counters%NGaussPoints), stat = IError) ! --> global matrix
    ...
    if (allocated(SigmaEffArrayGaussPoints) ) then
        deallocate(SigmaEffArrayGaussPoints, stat = IError)
    end if

    if (allocated(SubElementMPOrganization) ) then
        deallocate(SubElementMPOrganization, stat = IError)
    end if
    ...
    case(QUAD4)
        ! Could make all these subroutines into one statement ParticleStatus(:) = .true. - WaveHello
        ! Set the particle status
        select case (NParticles)
        case(1) ! 1 material point per element / FEM calculation
            call DetermineAdjacentParticlesQUAD4_MP1(NParticles, ParticleStatus)
        case(4) ! 4 material points per element
            call DetermineAdjacentParticlesQUAD4_MP4(NParticles, ParticleStatus)
        case(9) ! 9 material points per element
            call DetermineAdjacentParticlesQUAD4_MP9(NParticles, ParticleStatus)
        case(16) ! 1 material point per element / FEM calculation
            call DetermineAdjacentParticlesQUAD4_MP16(NParticles, ParticleStatus)
        case(25) ! 4 material points per element
            call DetermineAdjacentParticlesQUAD4_MP25(NParticles, ParticleStatus)
        case(36) ! 9 material points per element
            call DetermineAdjacentParticlesQUAD4_MP36(NParticles, ParticleStatus)
        case(49) ! 1 material point per element / FEM calculation
            call DetermineAdjacentParticlesQUAD4_MP49(NParticles, ParticleStatus)
        case(64) ! 4 material points per element
            call DetermineAdjacentParticlesQUAD4_MP64(NParticles, ParticleStatus)
        case(81) ! 9 material points per element
            call DetermineAdjacentParticlesQUAD4_MP81(NParticles, ParticleStatus)
        end select
