module mod_SRMC
   use mod_SRMC_funcs, only: Get_strain_invariants, YieldFunction, MatVec
   use mod_state_params, only: Check_Unloading, Get_I_coeff, check4crossing, Get_M, Get_Dp, Update_GK
...
!##    ## ##     ## ##     ##  ######     ##     ##  ######  ##     ##
   ...
   subroutine SRMC_HSR(NOEL, G_0, nu, M_tc, N, D_min, h, alpha_G, alpha_K, alpha_D, D_part, G_s,&
      ...
      double precision:: DE(6,6), dSig_el(6), Sig_t(6), dEps_t(6), dEps_TT(6), &
         ...
         dSig1(6), dSig2(6), Epspt(6), tempdSig(6)
         ...
         select case(switch_plastic_integration)
          ...
          case(Substepping) ! switch is set to zero
            ...
            do while (T<1.0d0) !main loop
               ...
               tempdSig = dSig1-dSig2
               call TwoNormTensor(tempdSig, 6, dummyvar(1)) !||Delta Sigma||
