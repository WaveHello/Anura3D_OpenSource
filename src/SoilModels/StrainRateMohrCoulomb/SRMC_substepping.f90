module mod_SRMC_Substepping
    use mod_SRMC_funcs, only: MatVec, Get_invariants, Get_strain_invariants, &
                              Get_dP_to_dSigma, Get_dD_to_dEpsP, Get_dD_to_dI, DotProduct_2, Get_dEpsq_to_dEps, &
                              YieldFunction
    use mod_yield_function, only: Get_dF_to_dSigma
    use mod_state_params, only: check4crossing, Get_dp, Update_GK
...
contains
   ...
   subroutine Newton_Raphson(G, K, eta_y, Dp, G_0, nu, M, M_tc, No, D_min, h, D_par, &
    ...
    ! Set the initial values for the alpha terms
    dI_alpha=alpha*dI
