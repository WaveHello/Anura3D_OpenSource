module MOD_SRMC_ESM
   ...
   public ESM_SRMC ! Overides private status for specific subroutine
contains
   Subroutine ESM_SRMC(NPT,NOEL,IDSET,STRESS,EUNLOADING,PLASTICMULTIPLIER, DSTRAN,NSTATEV,STATEV,NADDVAR,ADDITIONALVAR,CMNAME,NPROPS,PROPS,NUMBEROFPHASES,NTENS)
      ...
      integer, intent(in) :: naddvar
      integer, intent(in) :: npt
      integer, intent(in) :: noel
      integer, intent(in) :: idset
      integer, intent(in) :: numberofphases
      real(kind = Real_Type), dimension(ntens), intent(inout) :: stress
      real(kind = Real_Type), intent(inout) :: eunloading, plasticmultiplier
      real(kind = Real_Type), dimension(ntens), intent(inout) :: dstran
      real(kind = Real_Type), dimension(nstatev), intent(inout) :: statev
      real(kind = Real_Type), dimension(naddvar) :: additionalvar
      character(len=80) :: cmname !This is a parameter do not set intent
      real(kind = Real_Type), dimension(nprops), intent(inout) :: props
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

      !---Local variables required in standard UMAT
      integer :: IStep, TimeStep
      real(kind = Real_Type) :: sse, spd, scd ! specific elastic strain energy, plastic dissipation, creep dissipation
      real(kind = Real_Type) :: rpl ! only for fully coupled thermal analysis: volumetric heat generation
      real(kind = Real_Type), dimension(:), allocatable :: ddsddt ! only for fully coupled thermal analysis: variation of stress increment due to temperature
      real(kind = Real_Type), dimension(:), allocatable :: drplde ! only for fully coupled thermal analysis: variation of volumetric heat generation due to strain increment
      real(kind = Real_Type)                            :: drpldt ! only for fully coupled thermal analysis: variation of volumetric heat generation due to temperature
      real(kind = Real_Type), dimension(:), allocatable :: stran
      real(kind = Real_Type), dimension(:), allocatable :: time
      real(kind = Real_Type), dimension(:), allocatable :: predef
      real(kind = Real_Type), dimension(:), allocatable :: dpred
      real(kind = Real_Type), dimension(:), allocatable :: coords
      real(kind = Real_Type), dimension(:,:), allocatable :: ddsdde ! Jacobian matrix of the constitutive model (tangent stiffness matrix in case of MC)
      real(kind = Real_Type), dimension(:,:), allocatable :: drot
      real(kind = Real_Type), dimension(:,:), allocatable :: dfgrd0
      real(kind = Real_Type), dimension(:,:), allocatable :: dfgrd1
      real(kind = Real_Type) :: pnewdt, dtime, temp, dtemp, celent
      real(kind = Real_Type) :: Value ! auxiliary variable holding any real valued number
      real(kind = Real_Type) :: Porosity, WaterPressure, WaterPressure0, GasPressure, GasPressure0, DegreeSaturation
      ...
      !Call the UMAT
      call UMAT_NAMC(stress, statev, ddsdde, sse, spd, scd, rpl, ddsddt, drplde, &
   ...
   Subroutine UMAT_NAMC(STRESS, STATEV, DDSDDE, SSE, SPD, SCD, RPL, DDSDDT, DRPLDE, &
      ...
      ! Change the order of the Strain and Stress vectors to follow the convention of Anura3D
