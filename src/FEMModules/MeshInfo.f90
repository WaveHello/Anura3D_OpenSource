...
module ModMeshInfo
   ...
   use ModCounters, only: Counters, & ! Varaibles (Derived Type)
                          InitCountersFromFile ! Subroutine
                          
   use ModReadCalculationData, only: CalParams, IsDistorted, MinimumDeterminantRatioReached, & ! Variables
                                     IsFollowUpPhase, IsMPMComputation  ! Functions

   use ModElementEvaluation, only: GaussPointLocalCoordinates, ShapeFunctionData

   use ModGlobalConstants, only: REAL_TYPE, INTEGER_TYPE, N_NODES_HOE, IS3DCYLINDRIC, ISAXISYMMETRIC, N_BOUNDARY_NODES_HOE, &
                                 ELEMENTTYPE, ELEMENTBOUNDARYNODES, GOMunit, TETRAOLD, NDOFL, NVECTOR, ELEMENTGAUSSPOINTS, &
                                 ELEMENTNODES, MAXOUTPUTSURFACES, MAX_FILENAME_LENGTH, NDIM, FEEDBACK_LEVEL_ALWAYS
   
   use ModString, only: String ! TODO: Figure out what String can't tell if it's a module or type. It has interface procedures
   use ModFeedback, only : WriteInLogFile
   use ModFileIO, only: FExist
   ...
   real(REAL_TYPE), dimension(:, :), allocatable :: nodalWaterPressure ! mappping pressure from mps to nodes
...
contains ! Routines of this module
  ...
  !
  !    Description:
  !    Reads in data from GOM file related to the mesh including node and
  !    element connectivity matrices and associated fixities, at the same
  !    time it initiliazes counters and arrays to hold info related to
  !    the mesh.
  !
  !    @note: WaveHello - i think this subroutine contains too much
  !
  !**********************************************************************
  subroutine InitialiseMeshData()
  ...
  !    DescriptionTies edge nodes of an element to its corner nodes.
  ...
  !**********************************************************************
  !
  !    Subroutine:  TieHOENode
  !
  !    Description: Ties EdgeNode to CornerNode1 and CornerNode2.
  ...
  !    @note: 3D Function
  !********************************************************************** 
  subroutine TieHOENode(Connectivities, CornerNode1, EdgeNode, CornerNode2)

         implicit none
         ! TODO: Either create a integer for the size or make the size of the Connectivities array implicit
  ...
  !    
  !    Description:
  !    Deallocates the arrays used in this module.
  !
  !**********************************************************************
  subroutine DestroyMeshData()
         ...
         if (allocated(nodalWaterPressure)) then
            deallocate(nodalWaterPressure, stat = IError)
         end if
  ...
  !    Description:
  !    Determine which nodes belong to an active element
  !
  !    O: ActiveNodes(NodeID): logical array with length of ElementNodes
  !**********************************************************************
  subroutine DetermineActiveNodes()
  ...
  !
  !    Description:
  !    Calculates the number of active elements and sets logical
  !    array IsActiveElement
  !
  ! O   IsActiveElement: Logical array with length total_num_elements
  !                     with index set to true if that element is active
  !                     this time step
  ! O   Counters%NAEL  : Number of active elements
  ! !TODO: Might be able to use a mask and size to get the active elements count
  !**********************************************************************
  subroutine InitialiseNumberOfActiveElements()
         implicit none
  ...
  !**********************************************************************
  !
  !    Subroutine: sets determinant of each element
  !
  !    Description:
  !    Implemented in the frame of the MPM project.
  !
  ...
  !**********************************************************************
  !
  !    Function: HasDistortedElement
  !
  !    Description:  
  !    returns true when an element is distorted
  !    Implemented in the frame of the MPM project.
  !
  !**********************************************************************
  ...
  !**********************************************************************
  !
  !    Subroutine:  SetIsDistorted
  !    
  !    Description:
  !    Sets a global variable to true when an element is distorted
  !
  !    note: Implemented in the frame of the MPM project.
  !
  !**********************************************************************
  ...
  subroutine InitialiseNodalArrays()
         ...
         allocate(nodalWaterPressure(Counters%N, Counters%NEntity), stat = IError)
         nodalWaterPressure = 0.0
