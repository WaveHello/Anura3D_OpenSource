...
contains ! Routines of this module
   ...
   !**********************************************************************
   !
   !>   SUBROUTINE: GetNSideNodes
   !>   DESCRIPTION: 
   !>   Returns the number of nodes on each element side
   !!
   ...
   subroutine ShapeFunctionData(LocPos, IElTyp, ShapeValues, DShapeValues)
      !**********************************************************************
      !
      !    Subroutine:  Evaluates the shape functions and shape function derivatives
      ...
      ! Updated elements are handled by the ShapeLocPosPointer
      ...
      ! Assign the pointer values to the inout variables
      ! Pointer assigned values in InitialiseElementType.for file
   ...
   !
   !    SUBROUTINE: BMatrix
   !    DESCRIPTION: Determination of the strain interpolation matrix (B matrix)
   ...
   subroutine BMatrix(LocPos, IElTyp, NEl, NodTot, IDim, IElement, ICon, NodeCoord, B, DetJac)
      ...
      do J = 1, IDim
         do I = 1, IElTyp
            do K = 1, IDim
               B(K, I) = B(K,I) + (RJacInv(K, J) * dHS(I, J)) !(1/DetJac)
            end do
         end do
      end do
   ...
   !**********************************************************************
   !
   !    Subroutine: GetLocalCoordinates  
   !    Description: 
   !                Determination of local coordinates from global coordinates,
   ...
   !**********************************************************************
   !
   !    SUBROUTINE:  IsCornerNode
   !    DESCRIPTION:
   !    Returns .true. if INode is a corner node.
   !
   ...
   subroutine InitialiseShapeFunctions()
      ...
      ! we need the element gauss points to be 4 if we were to do the MG22 mixed integration scheme
