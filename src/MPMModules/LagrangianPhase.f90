...
contains ! Routines of this module
   ...
   subroutine LagrangianPhase()
      ...
      if (NFORMULATION==1) then
         ...
         end if ! End if CalParams%NumberOfPhases==2 .or. if  CalParams%NumberOfPhases==3 (End of single point liquid acceleration update)
         ...
         end if ! End if CalParams%NumberOfPhases==3 (End of single point three phase gas acceleration update)
         ...
         !------------- START 2-phase calculation (solid+liquid) ---------------------------
         if (CalParams%NumberOfPhases==2) then
            ...
            end if !End if(CalParams%ApplyFixedSolidSkeleton)
            !------------- END 2-phase calculation (solid+liquid) -----------------------------

            !------------- START 3-phase calculation (solid+liquid+gas) -----------------------
            ...
            !------------- END 3-phase calculation (solid+liquid+gas) -------------------------

            !------------- START 1-phase calculation (solid) ----------------------------------
         ...
         end if ! End if(CalParams%NumberOfPhases== 2, 3, 1)
         !------------- END 1-phase calculation (solid) ------------------------------------
         ...
         elseif (CalParams%NumberOfPhases==3) then ! Unsat Calculation (gas)
         ...
         end if ! End Correct Liquid and SOlid acceleration if there is an infiltration/seepage boundary condition
      end if ! End if NFORMULATION==1 (equivalent to NumbOfLayers=1)
      ...
      if (NFORMULATION==2) then
         if(CalParams%NumberOfPhases==1) then
            ...
            end if ! End Fixed Skeleton Condition
         ...
         end if ! End if NumberOfPhases=2
      end if ! End if NFORMULATION==2 (equivalent to NumbOfLayers=2)
   end subroutine LagrangianPhase

   subroutine GetNodalPressure(NodalPressure)
      !**********************************************************************
      !
      ! Function:  To map water pressure from particles to the grid points (nodes)
      !
      ! O  Momentum : Nodal pressure vector, the output of this subroutine
      ! Original Author: Alsardi
      ! Creation Date: 2023
      ! Last Modified: 11/23/2023
      !**********************************************************************
      implicit none
      real(REAL_TYPE), dimension(Counters%N,Counters%nEntity), intent(inout) :: NodalPressure
      ! Local variables
      integer(INTEGER_TYPE) :: I, IEl, IPart, INode, Ni, ParticleIndex, NodeID, iEntity
      real(REAL_TYPE) :: MPPressure, mass, shape
      NodalPressure = 0.0
      iEntity = 1
      do IEl = 1, Counters%NEl ! loop over all elements
         do IPart = 1, NPartEle(IEl) ! loop over all material points in element
            ...
            MPPressure = Particles(ParticleIndex)%WaterPressure        !Water pressure of the particle
            ...
            do INode = 1, ELEMENTNODES ! loop over nodes
               ...
               do I = 1, NVECTOR
                  ...
                  if (LumpedMassDry(Ni,iEntity)/=0) then
                     NodalPressure(Ni,iEntity) = NodalPressure(Ni,iEntity) + (Mass/ LumpedMassDry(Ni,iEntity)) * Shape * MPPressure
                  else
                     NodalPressure(Ni,iEntity) = 0.0
                  end if
               end do
   ...
   end subroutine GetNodalPressure

   subroutine MapNodalPressureBackToMPs
      !**********************************************************************
      !
      ! Function:  To map water pressure from grid points (nodes) back to the particles
      !
      ! O - Only Global variables updated
      !
      ! Original Author: Alsardi
      ! Creation Date: 2023
      ...
      real(REAL_TYPE), dimension(NVECTOR) :: ParticleWaterPressure
      real(REAL_TYPE), dimension(ELEMENTNODES, Counters%nEntity, NVECTOR) :: NodWP
      ...
      do IAEl = 1, Counters%NAEl ! Loop over all elements
         ...
         do I = 1, NVECTOR
            ...
            NodWP(1:ELEMENTNODES, 1:NoEn,I) = NodalWaterPressure(IDof(1:ELEMENTNODES, I), 1:NoEn)
         end do
         do IPart = 1, NPartEle(IEl)  ! Loop over all particles in element
            ...
            if((NFORMULATION==1).or. & (MaterialPointTypeArray(ParticleIndex)==MaterialPointTypeLiquid)) then
               ...
               do I = 1, NVECTOR ! Loop over dimensions
                  do INode = 1, ELEMENTNODES ! loop over element nodes
                     ...
                     ParticleWaterPressure(I) = ParticleWaterPressure(I) + ParticleShape(INode) * NodWP(INode, iEntity, I)
            ...
            Particles(ParticleIndex)%WaterPressure =  ParticleWaterPressure(1)       !Water pressure of the particle
            ParticleWaterPressure = 0
         end do !Loop over particles
      end do !elements
   end subroutine MapNodalPressureBackToMPs
