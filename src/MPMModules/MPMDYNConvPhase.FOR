...
contains ! Routines of this module
   ...
   subroutine DYNConvectivePhase()
      ...
      ! Update 1 MPM Mixed
      if (IsMPMWithMixedIntegration() .and. .not.IsMPMSkipConvection() .and. .not. CalParams%UpdatedMPMwithMixed) then ! Not needed for pure MPM and FEM  or Updated MPM Mixed
      ...
      ! Update 3 MPM Mixed
      if (IsMPMWithMixedIntegration() .and. .not.IsMPMSkipConvection() .and. .not. CalParams%UpdatedMPMwithMixed) then ! Not needed for pure MPM and FEM and for Updated MPM Mixed
         call StressAndPorePressureSmoothening()
         call StateParametersSmoothening ()

      else if (IsMPMWithMixedIntegration() .and. .not.IsMPMSkipConvection() .and.  CalParams%UpdatedMPMwithMixed) then
         call StressAndPorePressureSmoothening() ! If using updated MPM-Mixed only smooth the stresses
   ...
   subroutine UpdateParticleVelocityAndMapMomentum(Momentum)
      ...
      ! Local variables
      integer(INTEGER_TYPE) :: I, IEl, IAEl, IPart, INode, iEntity, ParticleIndex, NoEn, iSide, NID1, NID2
      ...
      real(REAL_TYPE) :: Time, ParticleMass, element_char_length, CoorDifference, Aux, waveSpeed
      logical:: IsInContactBoundary = .false.
      ...
      do IAEl = 1, Counters%NAEl   ! Loop over all elements
         ...
         do IPart = 1, NPartEle(IEl)   ! Loop over all particles in element
            ...
            if ( (MaterialPointTypeArray(ParticleIndex)==MaterialPointTypeSolid) .or. (NFORMULATION==1) ) then ! Check that the MP represents a solid or that the formulation is Mixed formulation
               ...
               ParticleVelocity = ParticleVelocity + ParticleIncrementalVelocity !Updated particle velocity
   ...
   subroutine StressAndPorePressureSmoothening()
      ...
      do IAEl = 1, Counters%NAEl
         ...
         if (CalParams%ApplyEmptyElements) then ! Applies to empty and partially filled elements
            AverageWeight = 0.0
            if (NMaterialParticlesEle(IEl)>0) then ! Check if the element is actually empty or partially filled
