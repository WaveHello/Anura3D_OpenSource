    ! PUT this information in the read contact data at leat ideally in the read gom file

    ! Loop over the GOM file and read the contact properties
    ! Contact properties are initialized using elements
    ...
    subroutine ContactSurfaceNodes_2D(FileUnit)
        ...
        character(len=64), dimension(:,:), allocatable :: material_props   ! array including the Contact Material name
        ...
        integer(INTEGER_TYPE) :: IError, ISurf, J, NContSur, NodID, I, IEl, NPileelements, NFoundNodes, CheckNodeID,  &
            ...
            CheckNodeI, II, num_contact_surfaces, global_material_id, NeighbourElement, adjacent_element_material_id,     &
            current_element_material_id, local_side_id, IRef, JRef, KRef, NRef
        ...
        integer(INTEGER_TYPE) :: width_ContactVolumes = 10 ! Has  width of 10 because it stores the Global element id (column 1), number of contacting materials for that element (column 2),
        ! the friction coeff and the adhesion for each of the 4 possible materials (2 * 4 = 8 values - other 8 columns)
        integer(INTEGER_TYPE) :: NFoundSides, start_element_row
        integer(INTEGER_TYPE), allocatable :: vertex_nodes(:, :), side_vertex_ids(:), num_vertex_per_side
        integer(INTEGER_TYPE), dimension(:,:), allocatable :: ContSurfaceNodes, ContVolSurf
        integer(INTEGER_TYPE), dimension(:)  , allocatable :: contact_surf_element_id, BoundaryElements, Sides, NNodes, MatNodes
        ...
        do
            ...
            if (trim(file_line)=='$$START_BODY_CONTACT_2D') then
                ...
                allocate(material_props(num_contact_elements, max_num_contact_materials ), stat=IError)  ! Storage for the contact material names
        ...
        ! Allocate variables
        allocate(Element(Counters%NEl), stat=IError)         ! Allocate an array to hold the element ids?? TODO: Check that this note is right WaveHello
        ...
        num_vertex_per_side = SHAPE(vertex_nodes, 1)
        ...
        do contact_material_index = 1, max_num_contact_materials  !loop over contacting materials
            ...
            do JMat = 1, CalParams%NumberOfMaterials  !loop over all materials
                if (material_props(1,contact_material_index)==MatParams(JMat)%MaterialName) then
                    ContactVolumesDummy(:, material_column_name) = ContactVolumes(:,material_column_name)
                end if
        ...
        do IEl = 1, num_contact_elements
            ...
            NPileelements = NPileelements+1 ! increment the number of pile elements
            ...
            ! NOTE: previously the absolute value of ElementMaterialID(global_element_id) was done. This doens't make any sense removed WaveHello
            PileEntityMaterial = ElementMaterialID(global_element_id)
            ...
            if  (IsActiveElement(global_element_id)) then
                ...
                do ISide = 1, ELEMENTSIDES
                    ...
                    ! Get the local node ids that make up the side
                    local_side_node_ids = vertex_nodes(:, ISide)
                    ...
                    if (is_side_on_free_surf==1) then
                        ...
                        NeighbourElement = GetAdjacentElement(global_element_id,ISide)
                        ...
                        if ((NeighbourElement/=0).or.(NeighbourElement/=-999)) then
                            ...
                            adjacent_element_material_id = ElementMaterialID(NeighbourElement)
        ...
        ! Loop over the element sides
        do II = 1, num_element_sides * num_contact_elements
            ...
            if (ContVolSurf(II,1)/=0) then            
                ...
                ! Get the element id
                element_id = II/num_element_sides
                
                ! Store the global element id and the number of contacting materials for that element
                ContactSurfaces(num_contact_surfaces,1:2) = ContactVolumes(element_id, 1:2)
                
                ! Store the global vertex (node) id
                ContactSurfaces(num_contact_surfaces,3:4) = ContVolSurf(II, 1:num_vertex_per_side)
                
                ! Store all of the contact material properties
                ContactSurfaces(num_contact_surfaces,5:12) = ContactVolumes(element_id, 3:10)
            end if
        end do

        ! Allocate variables
        allocate(IsContactSurface(NContSur), stat=IError)
        allocate(ContSurfaceNodes(NContSur, num_vertex_per_side), stat=IError)
        allocate(contact_surf_element_id(NContSur), stat=IError)
        ...
        ContSurfaceNodes(1:NContSur, 1:2)  = ContactSurfaces(1:NContSur, 3:4)
        
        
        do ISurf= 1, NContSur          ! Loop local surface ids
            do IEl = 1, Counters%NEl   ! Loop over all the elements
                if (Element(IEl)) then ! If element is in the contact elements
        ...
        allocate(MatNodes(NContSur), stat = IError) ! Material Nodes
        allocate(MatNodesCood(NContSur,2), stat = IError) ! Material Node Coordinates
        ...
        ! InterfaceNodes is the result that i need
